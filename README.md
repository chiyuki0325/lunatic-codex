...
- agent 工作或会话尚未配置时，将命令与普通消息分开处理：Enter 立即执行允许的斜杠命令，但把普通用户消息排队到当前 turn 结束；Tab 保持显式排队。无任务时 Enter 正常提交消息。
- 保留现有 `/model` 命令名，不新增 `/models` 别名。

### 非目标

- 不在当前 turn 的 tool call 边界切换模型、推理强度、权限或 personality。
- 不改变队列中普通用户消息在当前 turn 结束后的发送协议。
...
- agent 工作期间，Enter 输入普通消息会进入队列，Enter 输入允许的斜杠命令会立即执行；Tab 排队行为和当前禁止命令的错误提示保持不变。
