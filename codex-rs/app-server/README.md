...
- `thread/contextUsage` — experimental; read the current thread's context-usage snapshot without starting a turn, writing rollout items, or changing thread state. The request takes `threadId`; the response returns a structured snapshot containing `snapshotId`, request sequence, generation time, model name, optional model context window, optional auto-compact threshold, optional reserved-token budget, estimated category totals (`systemPrompt`, `builtInTools`, `mcpTools`, `instructions`, `skills`, `messages`, `other`, `unattributed`), optional MCP/instruction/skill detail rows (`label`, nullable `path`, `loadState`, `estimatedTokens`), snapshot completeness, and optional actual upstream token usage plus whether that actual value came from the current request, the previous completed request, or a local estimate. `lastCompletedSnapshotId` identifies the latest completed request snapshot associated with actual usage when known.
